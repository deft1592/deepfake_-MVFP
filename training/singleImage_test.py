...
FREPDD_GRADCAM_DIR = os.path.join(FREPDD_DEBUG_DIR, 'grad_cam')
...
def main():
    ...
    os.makedirs(FREPDD_GRADCAM_DIR, exist_ok=True)
    input_image_path = 'images/frepdd_img/grad_cam/000.png'          # <<< 改成你的图片路径
    ...
    out_perturbation = ts2image(output['perturbation'])
    out_perturbation_image=ts2image(output['perturbed_image'])

    out_perturbation.save(os.path.join(FREPDD_DEBUG_DIR, 'x_fake_perturbation.png'))

    out_perturbation_image.save(os.path.join(FREPDD_DEBUG_DIR, '000_p.png'))
