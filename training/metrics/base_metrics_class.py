...
class Recorder:
    ...
    def update(self, item, num=1):
        if item is not None:
            # Recorder is for scalar logging only. Keeping a loss tensor here
            # retains its complete autograd graph until the recorder is reset.
            if torch.is_tensor(item):
                if item.numel() != 1:
                    raise ValueError("Recorder only accepts scalar tensors")
