...
class Trainer(object):
    ...
    def cleanup_epoch_resources(self, train_data_loader):
        """每个epoch结束后清理资源"""
        try:
            
            if hasattr(train_data_loader.dataset, 'editor'):
                if train_data_loader.dataset.editor is not None:
                    # 重新初始化editor以避免内存泄漏
                    train_data_loader.dataset.editor.cleanup()
                    train_data_loader.dataset.editor = DECALightEditor()
    ...
    def train_step(self, data_dict):
        if self.config["optimizer"]["type"] == "sam":
            ...
            if (
                ...
                and self.config.get("lr_scheduler") == "cosine_warmup"
            ):
                self.scheduler.step()
    ...
    def train_epoch(
        ...
        for iteration, data_dict in tqdm(
            ...
            # 在这里添加释放dataset 和DECA 的缓存的函数
            
            #self.cleanup_epoch_resources(train_data_loader)

            # Do not keep the last training graph alive during validation.
            del losses, predictions, batch_metrics, data_dict
            
            # In DDP all ranks must stop training while rank 0 validates. Rank 0
            # uses the unwrapped module in inference(), so validation does not
            # wait for DDP collectives from the other ranks.
    ...
    def save_best(
        ...
        if improved:
            ...
            save_ffpp_ckpt = self.config.get("save_ckpt_on_ffpp", False)
            if self.config["save_ckpt"] and (key not in FFpp_pool or save_ffpp_ckpt):
    ...
    def test_epoch(self, epoch, iteration, test_data_loaders, step):
        ...
        for key in keys:
            ...
            losses_one_dataset_recorder, predictions_nps, label_nps = (
