...
class Trainer(object):
    ...
    def test_one_dataset(self, data_loader):
        ...
        for i, data_dict in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            # 使用 append + concatenate 避免 list 反复扩展复制的内存开销
            label_lists.append(data_dict["label"].cpu().detach().numpy())
            prediction_lists.append(predictions["prob"].cpu().detach().numpy())
        ...
        # 拼接后立刻释放中间 list，降低内存峰值
        pred_arr = np.concatenate(prediction_lists)
        label_arr = np.concatenate(label_lists)
        feat_arr = np.concatenate(feature_lists)
        del prediction_lists, label_lists, feature_lists
        return (
            test_recorder_loss,
            pred_arr,
            label_arr,
    ...
    def test_epoch(self, epoch, iteration, test_data_loaders, step):
        ...
        # 显式释放测试大对象并清空 CUDA 缓存，防止内存峰值累积
        del losses_all_datasets, metrics_all_datasets
        torch.cuda.empty_cache()
        return (
