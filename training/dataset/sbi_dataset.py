...
class SBIDataset(DeepfakeAbstractBaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # fake_image_path, fake_label = self.fake_imglist[index]

            # # Get the landmark paths for fake images
        # fake_landmark_path = fake_image_path.replace("frames", "landmarks").replace(
        #     ".png", ".npy"
        # )
        # landmark = self.load_landmark(fake_landmark_path).astype(np.int32)

        # # Load the fake images
        # fake_image = self.load_rgb(fake_image_path)
        # fake_image = np.array(fake_image)  # Convert to numpy array  
