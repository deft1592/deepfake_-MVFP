...
def get_boundary(mask, apply_dilation=True, apply_motion_blur=True):
    ...
    if mask.max() > 1:
        ...
        boundary = mask
    boundary = 4 * boundary * (1.0 - boundary)
...
class SBI_API:
    def __init__(self, phase="train", image_size=256):
        ...
        # assert (
        #     phase == "train"
        # ), f"Current SBI API only support train phase, but got {phase}"
    ...
    def __call__(self, img,  editor,landmark=None):
        try:
            assert (
                landmark is not None
            ), "landmark of the facial image should not be None."
            # img_r,img_f,mask_f=self.self_blending(img.copy(),landmark.copy())

            if random.random() < 1.0:
                # apply sbi
                img_r, img_f, mask_f = self.self_blending(img.copy(), landmark.copy(),editor)#必定执行这一步
            else:
    ...
    def self_blending(self, img, landmark,editor):
        H, W = len(img), len(img[0])
        if np.random.rand() < 0.25:
            landmark = landmark[:68]
        # mask=np.zeros_like(img[:,:,0])
        ...
        if np.random.rand() < 0.1:
            ...
            id= random.choice(list(LIGHT_DIRECTIONS.keys()))
            light_direction = LIGHT_DIRECTIONS[id]
            randomIntensity=random.uniform(2.5,3.0)
            print(f"Apply LSM with current_device{ editor.device}")
            source = editor.apply_3d_informed_lighting_with_computed_normals(source, light_direction, mask,randomIntensity)
