...
def random_get_hull(landmark,img1,hull_type):
    ...
    #随机resize hull 
    # 计算所有关键点的质心
    centroid = np.mean(landmark, axis=0)
    scale_factor_list=[0.9,1.0,1.1]
    scale_factor=random.choice(scale_factor_list)
    # 计算每个关键点到质心的向量，并随机扩大alpha 倍
    scaled_landmark = []
    for point in landmark:
        ...
        scaled_landmark.append(new_point)
    
    # 转换为numpy数组
    scaled_landmark = np.array(scaled_landmark).astype('int32')
